...
AI-driven PPT generation system. The Agent reads the framework guidance, makes
...
| --- | --- | --- |
| `core` | `npm test` and the compatible `ppt_flow test` command run the bounded protected core inventory | Every normal framework change |
...
| Directory | Purpose |
| --- | --- |
| `PPTMAKER_FRAMEWORK/` | Methodology knowledge base and production scripts |
| `tests/` | Unit and integration tests |
| `tests_e2e/` | End-to-end tests |
| `openspec/` | Spec-driven development |
| `_backlog/` | Backlog, bugs, and plans |
| `deck_*/` | Generated run bundles, not framework source |
| `dpt_*/` | Deep-research input; do not read unless it is named |

`deck_*` is created for presentation work and is production data, not framework
source. Framework maintenance changes the framework, specs, and tests; deck
