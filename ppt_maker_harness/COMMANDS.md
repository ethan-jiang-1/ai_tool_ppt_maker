...
| "Keep this unpublished first version and redo the pages." | Confirm the deck still holds only its first unpublished page structure, with no provider images or PPTX | The first structure restored to an unpublished draft, then a fresh page plan as that same first version | Confirm abandoning the current unpublished structure | Short local work |
| "Which image channel is working?" | Whether the Image2 Call Shape is already confirmed or still a candidate | Confirmed: the Agent runs a short connectivity check and shows you the report. Candidate: the Agent discovers a working Call Shape through the Image2 Lab playbook and its Lab CLI. An empty `_lab/` workspace does not block drawing when a confirmed Call Shape exists. | Live work needs an exact disclosed submit count and your confirmation. A connectivity success is not permission to generate. | Local check or provider-variable work |
