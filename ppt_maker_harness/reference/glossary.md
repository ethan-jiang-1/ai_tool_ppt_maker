...
| accepted Style Master selection | `_state/state.yaml` `page_authority_style_master.by_version["3_versions/vN"]` | Optional schema-v5 acceptance record and raw-profile authority; first vNext has no inherited target record. |
...
Selection/intent/context/profile/byte drift and legacy raw lineage without the
