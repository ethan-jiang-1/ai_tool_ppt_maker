...
position: quickstart
type: guide
summary: First-time user entry for Page Image deck production.
depends_on:
- README.md
feeds_into:
- AGENTS.md
...
help turn the idea into a clear narrative and stable Page Image slide IDs.
...
| --- | --- |
| Claim, audience, examples, and visual direction | Creates the run bundle and Page Image source |
...
Framed uses a deterministic local header overlay while the Provider renders the
body. Pure uses the Provider for all final pixels. The Agent records the required
human decisions at Complete Page Review and delivery review through `03-framed-image XOR 04-pure-image ->
