...
function assertReplacementAssemblyRecord(record, kind, path = null) {
...
export function validatePageImageAssemblyInput({ manifest, acceptedRawEvidence, sourceEpoch } = {}) {
  assertReplacementAssemblyRecord(manifest, "final-manifest");
  assertReplacementAssemblyRecord(acceptedRawEvidence, "accepted-raw-evidence");
...
function readPersistedFinalManifest(paths, input) {
  ...
  assertReplacementAssemblyRecord(bytes, "final-manifest", paths.target_final_manifest);
  let manifest;
  try {
