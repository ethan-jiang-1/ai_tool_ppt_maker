...
} from "./page_authority_visual_language.mjs";

export const PAGE_AUTHORITY_REFERENCE_REGISTRY_SCHEMA = "pptmaker-image2-reference-registry-v1";
export const PAGE_AUTHORITY_REFERENCE_ROOT = "2_backbone/visual-style/assets/reference";
...
export class PageAuthorityReferenceMaterialError extends Error {
  constructor(issues) {
    const list = Array.isArray(issues) ? issues : [issues];
    super(list.map((item) => item.message || String(item)).join("; "));
    this.name = "PageAuthorityReferenceMaterialError";
...
function parseString(node, context, issues, { id = false, sha = false, textGuard = false } = {}) {
  if (!plainString(node)) {
...
function parseRoles(node, context, issues) {
  ...
  for (const pair of node.items) {
    ...
    const roleClause = parseString(role.get("role_clause"), `${context}.${roleId}.role_clause`, issues, { textGuard: true });
    if (
      !referencePath
...
export function parsePageAuthorityReferenceMaterial(raw, { expectedProfile = null } = {}) {
  let document;
  try {
    document = parseDocument(raw, { version: "1.2", schema: "core", uniqueKeys: true, merge: false, keepSourceTokens: true });
  } catch (error) {
    throw new PageAuthorityReferenceMaterialError(problem("invalid_reference_yaml", error.message));
  }
  ...
  if (schema !== PAGE_AUTHORITY_REFERENCE_REGISTRY_SCHEMA) issues.push(problem("invalid_reference_schema", `reference registry.schema must equal ${PAGE_AUTHORITY_REFERENCE_REGISTRY_SCHEMA}`, { actual: schema, expected: PAGE_AUTHORITY_REFERENCE_REGISTRY_SCHEMA }));
  ...
  if (issues.length > 0) throw new PageAuthorityReferenceMaterialError(issues);
  return deepFreeze({ schema, profiles });
}

function profileDirectory(deckDir, profileId) {
  if (!LOWER_KEBAB_ID.test(profileId || "")) throw new PageAuthorityReferenceMaterialError(problem("invalid_reference_id", "profile ID must be a lower-kebab ID", { actual: profileId }));
  const root = resolve(deckDir, PAGE_AUTHORITY_REFERENCE_ROOT);
  ...
  if (!relation || relation.startsWith(`..${sep}`) || relation === ".." || isAbsolute(relation)) {
    throw new PageAuthorityReferenceMaterialError(problem("reference_path_escape", "profile directory escapes the Page Authority reference root", { actual: profileId }));
  }
  return directory;
}
...
function verifyAmberModelSheet(profileId, directory) {
  ...
  if (!existsSync(path)) throw new PageAuthorityReferenceMaterialError(problem("missing_model_sheet", "amber-agent doctrine model sheet is missing", { path }));
  const actual = sha256(readFileSync(path));
  if (actual !== AMBER_AGENT_MODEL_SHEET_SHA256) {
    throw new PageAuthorityReferenceMaterialError(problem("amber_model_sheet_sha_mismatch", "amber-agent doctrine model sheet checksum differs from the verified v1 source", { path, actual, expected: AMBER_AGENT_MODEL_SHEET_SHA256 }));
  }
}

/** Load the sole selected profile registry, never an HTML catalog or version override. */
export function loadPageAuthorityReferenceMaterial(deckDir, profileId) {
  ...
  try {
    ...
    throw new PageAuthorityReferenceMaterialError(problem("reference_registry_unavailable", `could not read Page Authority image2 reference registry: ${error.message}`, { path }));
  }
  const material = parsePageAuthorityReferenceMaterial(raw, { expectedProfile: profileId });
...
export function resolvePageAuthorityIdentityReference({ deckDir, identity, identity_subject_count, subject_restrictions } = {}) {
  ...
  if (!LOWER_KEBAB_ID.test(profileId || "") || !LOWER_KEBAB_ID.test(roleId || "")) {
    throw new PageAuthorityReferenceMaterialError(problem("invalid_reference_identity", "identity profile and role must be lower-kebab IDs", { actual: identity }));
  }
  const material = loadPageAuthorityReferenceMaterial(deckDir, profileId);
  const profile = material.profiles[profileId];
  const role = profile?.roles?.[roleId];
  if (!profile) throw new PageAuthorityReferenceMaterialError(problem("unregistered_identity_profile", `VISUAL IDENTITY selects unregistered profile ${JSON.stringify(profileId)}`, { actual: profileId }));
  if (!role) throw new PageAuthorityReferenceMaterialError(problem("unregistered_identity_role", `VISUAL IDENTITY selects unregistered role ${JSON.stringify(roleId)}`, { actual: roleId }));
  if (identity_subject_count !== "one" || identity_subject_count > profile.maximum_identity_subjects) {
    throw new PageAuthorityReferenceMaterialError(problem("identity_subject_count_incompatible", `profile ${JSON.stringify(profileId)} permits exactly one identity subject`, { actual: identity_subject_count, expected: "one" }));
  }
  if (!RESTRICTIONS.includes(subject_restrictions) || profile.incompatible_restrictions.includes(subject_restrictions) || !profile.compatible_restrictions.includes(subject_restrictions)) {
    throw new PageAuthorityReferenceMaterialError(problem("identity_restriction_incompatible", `profile ${JSON.stringify(profileId)} is incompatible with ${JSON.stringify(subject_restrictions)}`, { actual: subject_restrictions, expected: profile.compatible_restrictions }));
  }
  ...
  if (relation.startsWith(`..${sep}`) || relation === ".." || isAbsolute(relation) || !existsSync(referencePath)) {
    throw new PageAuthorityReferenceMaterialError(problem("reference_path_escape", "selected role reference is unavailable outside its profile directory", { actual: role.reference_path }));
  }
  const actualSha = sha256(readFileSync(referencePath));
  if (actualSha !== role.reference_sha256) {
    throw new PageAuthorityReferenceMaterialError(problem("reference_sha_mismatch", `selected role ${JSON.stringify(roleId)} does not match its registered bytes`, { actual: actualSha, expected: role.reference_sha256 }));
  }
  const projection = {
    profile: profileId,
    role: roleId,
...
export function createPageAuthoritySourceResolver({ deckDir, visualLanguage } = {}) {
  if (!visualLanguage) throw new TypeError("visualLanguage must be a parsed Page Authority visual language registry");
  return Object.freeze({
    resolveSelection(context) {
      const identity = context.identity
        ...
        : null;
      const visualLanguageResolution = resolvePageAuthorityVisualLanguageSelection(visualLanguage, {
        ...context,
