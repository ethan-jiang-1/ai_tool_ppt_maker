/**
 * Current Page Image Workflow production policy.
 *
 * This module intentionally has one current vocabulary. Non-current source and
...
const TARGET_POLICY = Object.freeze({
  ...
  adapter: PAGE_IMAGE_WORKFLOW_ADAPTER,
});
...
export function productionPolicyForMode(mode) {
  if (mode === PAGE_IMAGE_WORKFLOW_PRODUCTION_MODE) return { ok: true, ...TARGET_POLICY };
  return {
    ok: false,
...
function readCurrentRecord(record, versionKey) {
  ...
  if (Object.keys(record).length !== expectedKeys.length || !expectedKeys.every((key) => Object.hasOwn(record, key)) ||
    ...
    return {
      ok: false,
      code: "PAGE_IMAGE_WORKFLOW_STATE_INVALID",
      version_key: versionKey,
      mode: record.mode,
      next_action: "repair_page_image_workflow_state",
    };
  }
...
export function isProductionModeRecord(record) {
  ...
  return record.mode === PAGE_IMAGE_WORKFLOW_PRODUCTION_MODE && Object.keys(record).length === 3 &&
    Number.isInteger(record.source_epoch) && record.source_epoch >= 1 &&
    PAGE_IMAGE_WORKFLOWS.includes(record.workflow) && Object.hasOwn(record, "mode") &&
    Object.hasOwn(record, "workflow") && Object.hasOwn(record, "source_epoch");
}

export function initialProductionModeRecord(mode = PAGE_IMAGE_WORKFLOW_PRODUCTION_MODE, workflow = null) {
  if (!isProductionMode(mode)) throw new TypeError("mode must be a supported Page Image Workflow mode");
  if (mode === PAGE_IMAGE_WORKFLOW_PRODUCTION_MODE) {
    if (!PAGE_IMAGE_WORKFLOWS.includes(workflow)) throw new TypeError("Page Image Workflow mode requires workflow framed | pure");
    return { mode: PAGE_IMAGE_WORKFLOW_PRODUCTION_MODE, workflow, source_epoch: 1 };
  }
}

/** Inspect only the one current Page Image Workflow source/state pair. */
export function inspectProductionMode({ state, runDir, runVersion, sourceMarker } = {}) {
  ...
  if (!policy.ok || marker.pipeline !== policy.pipeline || (record.mode === PAGE_IMAGE_WORKFLOW_PRODUCTION_MODE && marker.workflow !== record.workflow)) {
    return {
      ok: false,
