...
function assertDigest(value, field) {
  if (!SHA256_RE.test(value || "")) throw new PageImageArtifactError("invalid_digest", `${field} must be a lowercase SHA-256`);
}

function assertWorkflow(workflow) {
  if (!WORKFLOWS.includes(workflow)) throw new PageImageArtifactError("invalid_workflow", "workflow must be framed | pure");
}
...
/** Name one rebuildable human-facing page image from its current position and stable ID. */
export function pageImageOrdinalImageFilename(position, slideId) {
  if (!SLIDE_ID_RE.test(slideId || "")) {
    throw new PageImageArtifactError("invalid_slide_id", "slide_id must be a stable Page Image slide ID");
  }
  return `${formatPageImageOrdinal(position)}_${slideId}.png`;
}

function bytes(value, label) {
  if (!Buffer.isBuffer(value) && !(value instanceof Uint8Array)) throw new PageImageArtifactError("invalid_bytes", `${label} must be nonempty bytes`);
  const copy = Buffer.from(value);
  if (copy.length === 0) throw new PageImageArtifactError("invalid_bytes", `${label} must be nonempty bytes`);
  return copy;
}
...
export function validateRawWorkPlanProviderInputBindings(plan) {
...
export function createRawWorkPlan(input) {
  ...
  if (!validation.ok) throw new PageImageArtifactError(validation.code, validation.message);
...
export function validateAcceptedRawEvidence(evidence, { plan = null } = {}) {
  try {
    ...
    if (!Array.isArray(evidence.items) || evidence.items.length === 0) throw new PageImageArtifactError("raw_evidence_invalid", "accepted raw evidence needs ordered items");
    const seen = new Set();
    for (const item of evidence.items) {
      if (!exactKeys(item, ["slide_id", "raw_sha256", "path"]) || !SLIDE_ID_RE.test(item.slide_id || "") || seen.has(item.slide_id) || typeof item.path !== "string" || !item.path) {
        throw new PageImageArtifactError("raw_evidence_invalid", "accepted raw evidence item is invalid");
      }
    ...
    if (plan) {
      ...
      if (!checked.ok || checked.sha256 !== evidence.raw_work_plan_sha256 || plan.source_receipt_sha256 !== evidence.source_receipt_sha256 || plan.workflow !== evidence.workflow ||
        ...
        throw new PageImageArtifactError("raw_evidence_stale", "accepted raw evidence does not bind the current raw work plan");
      }
    }
...
export function createAcceptedRawEvidence({ plan, provider_authorization_sha256, raw_review_sha256, raw_bytes_by_slide } = {}) {
  const checkedPlan = validateRawWorkPlanProviderInputBindings(plan);
  if (!checkedPlan.ok) throw new PageImageArtifactError(checkedPlan.code, checkedPlan.message);
  if (!raw_bytes_by_slide || typeof raw_bytes_by_slide !== "object" || Array.isArray(raw_bytes_by_slide)) throw new PageImageArtifactError("raw_evidence_invalid", "raw_bytes_by_slide is required");
  const ids = plan.ordered_slide_ids;
  if (Object.keys(raw_bytes_by_slide).sort().join("\n") !== [...ids].sort().join("\n")) throw new PageImageArtifactError("raw_evidence_invalid", "raw bytes must cover the raw work plan exactly");
  const evidence = {
...
export function validateFinalSlideManifest(manifest, { evidence = null, expectedWorkflow = null } = {}) {
  try {
    ...
    if (expectedWorkflow && manifest.workflow !== expectedWorkflow) throw new PageImageArtifactError("wrong_workflow_owner", "final manifest workflow does not match its owner");
    if (!Array.isArray(manifest.items) || manifest.items.length === 0) throw new PageImageArtifactError("final_manifest_invalid", "final slide manifest needs ordered items");
    const ids = [];
    for (const item of manifest.items) {
      if (!finalManifestItemShape(item) || !SLIDE_ID_RE.test(item.slide_id || "") || !Number.isInteger(item.position) || item.position < 1 || item.path !== pageImageOrdinalImageFilename(item.position, item.slide_id) ||
        (Object.hasOwn(item, "width") && (!Number.isSafeInteger(item.width) || item.width <= 0 || !Number.isSafeInteger(item.height) || item.height <= 0))) {
        throw new PageImageArtifactError("final_manifest_invalid", "final slide manifest item is invalid");
      }
      assertDigest(item.final_sha256, "final_sha256");
      ids.push(item.slide_id);
    }
    if (new Set(ids).size !== ids.length || manifest.items.some((item, index) => item.position !== index + 1)) throw new PageImageArtifactError("final_manifest_invalid", "final slide manifest position/order is invalid");
    if (evidence) {
      ...
      if (!checked.ok || checked.sha256 !== manifest.accepted_raw_evidence_sha256 || evidence.source_receipt_sha256 !== manifest.source_receipt_sha256 || evidence.workflow !== manifest.workflow ||
        ...
        throw new PageImageArtifactError("final_manifest_stale", "final slide manifest does not bind current accepted raw evidence");
      }
    }
...
export function createFinalSlideManifest({ evidence, expected_workflow, final_bytes_by_slide, final_dimensions_by_slide = null } = {}) {
  ...
  if (!checkedEvidence.ok) throw new PageImageArtifactError(checkedEvidence.code, checkedEvidence.message);
  if (expected_workflow !== evidence.workflow) throw new PageImageArtifactError("wrong_workflow_owner", "only the selected workflow may publish final slides");
  if (!final_bytes_by_slide || typeof final_bytes_by_slide !== "object" || Array.isArray(final_bytes_by_slide)) throw new PageImageArtifactError("final_manifest_invalid", "final_bytes_by_slide is required");
  const ids = evidence.items.map((item) => item.slide_id);
  if (Object.keys(final_bytes_by_slide).sort().join("\n") !== [...ids].sort().join("\n")) throw new PageImageArtifactError("final_manifest_invalid", "final bytes must cover accepted evidence exactly");
  if (final_dimensions_by_slide !== null && (!final_dimensions_by_slide || typeof final_dimensions_by_slide !== "object" || Array.isArray(final_dimensions_by_slide) ||
    ...
    throw new PageImageArtifactError("final_manifest_invalid", "final dimensions must cover accepted evidence exactly");
  }
  const manifest = {
    ...
    items: ids.map((slide_id, index) => {
      ...
      if (dimensions && (!Number.isSafeInteger(dimensions.width) || dimensions.width <= 0 || !Number.isSafeInteger(dimensions.height) || dimensions.height <= 0)) {
        throw new PageImageArtifactError("final_manifest_invalid", `final dimensions for ${slide_id} are invalid`);
      }
      return {
        slide_id,
        position: index + 1,
        final_sha256: sha256Bytes(bytes(final_bytes_by_slide[slide_id], `final ${slide_id}`)),
        path: pageImageOrdinalImageFilename(index + 1, slide_id),
