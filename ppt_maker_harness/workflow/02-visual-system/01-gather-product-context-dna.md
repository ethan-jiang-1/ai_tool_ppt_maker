...
**Navigation**: ← `00-why-deterministic-visual-systems.md` | Next → `02-design-the-visual-system.md`
...
**从 DNA 推 medium（画风）——先于颜色的那一步。** 上表的 "Visual cues" 列其实指向一个比色板更根本的决定：**用什么媒介表达主要视觉**。把这些线索抽象成明确的 medium，它会成为 `02-design-the-visual-system.md` 的 **Dimension 0**，并约束 family、asset、abstract fallback 与 chart recipe：
...
如果 client 有 physical product 且具有 distinctive visual character，把一张经过许可、校验并登记的 **product reference asset** 放入 asset catalog。它用于 visual-focus/split 等明确消费资产的页面，展示 scale、surface finish、material 和 light behavior；它不是整册隐式 style reference。
...
如何获取 reference asset：
1. 从 client 网站、investor materials 或 technical documentation 找 reference
2. 从用户提供或已授权来源取得 byte-stable 本地资产
3. 在 catalog 中登记 ID、路径、SHA、kind 与适用语义；需要的 slide 显式引用它

Change 3 的默认路径不远端生成 reference asset。没有合适资产时，使用 renderer-owned abstract pattern、icon composition 或 structured fallback；不要为了补 visual DNA 自动调用 provider。
