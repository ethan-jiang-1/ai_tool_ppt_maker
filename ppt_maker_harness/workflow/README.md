...
New Page Authority work is version-homogeneous. After `01-content` records one
v2 workflow and `02-visual-system` supplies shared visual inputs, the target
method graph is:
...
| 01 | v2 source, stable slide identity, and workflow receipt |
| 02 | Visual language and shared reference inputs |
| 03 | Framed semantic rules, text-free underlays, local Text Frame composition |
...
semantics or delivery. A non-v2, partial, hybrid, or mismatched source/state
pair is a byte-preserving `unsupported-protocol/export` hard-stop, not an
