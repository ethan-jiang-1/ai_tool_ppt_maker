...
7. `guide|confirm|hard-stop` 的分类、waiver 与 protected invariant 继续由现有 gate policy/owner 定义。
...
| 每个 failure 的最近合法动作？ | 一个 action；hard-stop 说明不变量与恢复路径 |
...
`confirm` 仍需要明确的人类理由；`hard-stop` 不接受 force/waive 绕过。Inspection 的 result 是 projection，
...
- 同一 checkpoint 的 next-action evaluator 数量：目标 1。
...
- 一个 canonical task 跨越的 public seam：目标 1。
...
- **简化误删 safety gate**：先按 gate policy 分类；identity/integrity/auth/recovery 不降低；negative tests
  证明 valid work 不被 block。
- **inspection 变成第二 authority**：保持 read-only、无 cache、只组合 domain owner；writer mutation 前重新
  验证 direct fact。
...
- **目录改名留下双主概念**：Change 2 原子更新 runtime、main specs、tests 与入口文档；Change 3 只允许有
  owner/reason 的 compatibility exception，避免新旧 Phase 同时成为活动 truth。
