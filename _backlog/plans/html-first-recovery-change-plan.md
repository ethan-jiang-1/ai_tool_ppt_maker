...
**预计修改 capability**：`cli-surface`、`commands-reference`、`content-parsing`、
`html-slide-contract`、`html-slide-rendering`、`visual-config`、`visual-asset-management`、
`pipeline-orchestration`、`playbook-execution`、`node-specification`、`run-bundle-layout`、`run-bundle-management`。
...
- HTML production/migration topology 只忽略显式 allowlist 中的 `.DS_Store`，不泛化忽略所有
  dotfile，也不忽略 `.publish.lock`、journal 或未知隐藏文件；其他 run-bundle owner 不因该 bug
  被顺手重写；
...
- [x] **Change 1 - `make-html-production-guided-and-recoverable`**: **已完成并归档**。
  ...
  `266a7db`。
- [ ] **Change 2 - `complete-markerless-html-migration`**: **planning review 第 2 轮已完成，进入第 3 轮，唯一 active change**。
  proposal、design、12 个 capability delta specs 与 35 项未执行 implementation tasks 已生成；
  `openspec validate complete-markerless-html-migration --strict` 与 main-spec strict validation
  均已通过。第一轮将 projected-run 收敛为 candidate overlay（不再误作独立 deck），补齐 HTML
  contract/renderer、asset catalog 和 run-bundle layout 的责任边界；第二轮已识别并定义 state-owned
  receipt-bound confirmation，堵住 active apply execution 的预写入空档，并将 hidden target 收窄为
  inherited inputs + candidate source/overrides，禁止复制 legacy source tree 或 generated bytes。文档入口
  也统一为 prepare → confirmation → apply。第三轮进一步将 old-side evidence/mode drift 纳入 confirmation
  precondition，避免 comparison 改变后复用旧确认。第二、三轮 strict validation 均已通过；实施仍为
  `5/35`，candidate resolver/renderer context 与 synthetic markerless fixture 已验证，当前正在完成 isolated preparation；后续仍按 `tasks.md` 严格
  串行 apply；在 Change 2 归档前，不得开始 Change 3。
