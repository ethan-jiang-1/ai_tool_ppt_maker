...
> 本计划执行 **4 个 change（C0/C1/C2/C4,见 `progress.md`）**;**C3 延后（α）**。以下各项
> **有意不做或延后**,每条都记录理由与重新评估条件,以免被误读为遗漏。
> 修订（吸收 07）: exit-2 基线已纠正;F 的大部分随 C4 消解;v5（α）: C3 延后。

## 一、延后（第二梯队,本计划归档后重新评估）

| 项 | 内容 | 修订后状态 |
| --- | --- | --- |
| **C3 state/投影重建触发重设计**（α,v5 人类决定） | 把协作卡的三处隐式刷新改为显式 `task-projection rebuild` 命令 + `state` 回零写 | **延后**: trigger 点实测 ≈15–20 个 Controller 节点（create-deck 共 40 节点）;卡片是 spec 明确的非权威协作视图,价值小风险大;C1/C2/C4 不依赖它。设计预案保留在 `03`;摩擦由 C1 help 契约块兜底。**重启条件**: 四个 change 归档后,若 Agent 实际使用反馈显示「state 隐藏写」是高频痛点,按 `03` 预案重启（先重跑 trigger 计数） |
...
| findings-I 选项 E（治理转向） | closed schema 驱动命令生成 | 先有 C0/C1/C2/C4 的拆分成果,才知道 surface 该长什么样 |
...
- **exit 2 特例**: 属于普通 `state` 的 replacement/current-repair hard-stop
  （`ppt_flow.mjs:3869`）,搬移时保留——**不是** `state validate`（其 invalid 是 exit 1,`:3832`;
...
- **Task Mandate 权威**（对齐已归档 `fold-style-master-cost-into-task-mandate`,commit `5571002`）:
  ...
  若重启 C3 亦不得碰撞（见 `01` §1.8、`03` 变更形状）。

## 四、记录与重新评估

- 本清单在本 plan 关闭时随 plan 移入 `_done/_closed_plans/`,延后项若仍未排期,
  在 `_backlog/todos` 留一条「重新评估 C/B/C3」记录。
- 重新评估条件: **四个 change（C0/C1/C2/C4）全部归档** + 实际 Agent 使用反馈收集后,按
  [影响面] × [收益] 重排（含 C3 是否重启）,不提前承诺顺序。
