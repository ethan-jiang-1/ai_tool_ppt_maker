...
因此，研究中所有关于 source authority、provider-rendered content、prompt lineage、protected zone 和 Complete Page Review 的结论同时适用于 Pure/Framed 的共同核心；Framed 只增加 context-not-to-render、protected geometry、local transparent overlay 与 raw/composite 双视图的差异性约束。
...
Owner 已决定：`provider_rendered_content` 默认要求 exact literal；只有 source 显式标记的非事实性 supporting copy 可作为 Presentation-Adaptable Copy 让 Image2 为图文一体构图压缩或改写，不能由模型自行判断。该授权不覆盖 claims、facts、numbers、names、labels、headers 或未标记文案。结构化字段应能表达 role、literal policy、exact text/data、与 primary claim 的关系，但不暴露自由坐标和 transport prompt。
...
因此主计划不能承诺“固定 header 内容变化继续本地刷新”。应改为：**provider-input-preserving local header refresh**。Owner 已决定把 exact header literals 全部作为 `render=false` context 发送给 provider；任何 literal 改动都自然改变 prompt digest 并重建 raw，这是获得更强叙事一致性的明确代价。

## 验收模型

Owner 已确认“三层检查、两类主要人工决定”，避免新增无谓 gate：

1. **Deterministic preflight（机器）**：source schema、exact literals、frame fit、geometry、contract/prompt digest、coverage。
2. **Complete Page Review（人）**：沿用 Pilot/complete lifecycle，作一次 `proceed / repair` 决定。Pilot 仍是样本/成本 gate；Framed 的 complete review 并列展示 exact raw 与 production-equivalent composite，Pure 则展示完整 provider page。两种模式检查必需文字逐字正确、数据正确和整体构图；Framed 额外检查无 fixed header 重复与 protected-zone 避让。
...
Framed complete review 已展示 production-equivalent composite，因此不需要新增独立“composite approval”状态；Pure 也不需要凭空创建 composite。delivery review 仍绑定最终交付 lineage。

## Clean Replacement Boundary

1. `page-authority-image2-v2` 是错误模型，不保留其 source、state、receipt 或 evidence 的 active interpretation。
2. 正确模型使用自己的 current protocol identity；active Harness 删除 v2 route，且不接受 v2 marker、state、receipt、raw evidence 或 delivery evidence。
3. 不存在 v2-to-current shim、capability gate、fallback、refresh 或 conversion path。正确模型从新的 canonical source、receipt、authorization、raw generation、review 和 delivery lineage 开始。
4. 不批量迁移 deck，也不继承旧 raw acceptance/delivery decision。是否删除旧数据是独立的显式数据清理动作。
5. prompt compiler ownership 修复属于正确模型的新 lineage；实际发送 prompt 不得沿用任何错误模型的 plan hash 或 authorization。
...
6. 把两级视觉验收细化为 deterministic preflight + Complete Page Review（Framed raw/composite 并列、Pure 完整 provider page）+ final delivery review；Pilot 是样本成本阶段，不是第三套语义 acceptance。
7. 把 `SLIDE BODY` 从“可能成为 canonical input”改为“候选语法 seam；需先定义领域 schema，不能直接接线”。

## 已确认的审查与视觉决定

Complete Page Review 使用单一 `proceed / repair` 决定：Framed 并列展示 raw 与 production-equivalent composite，Pure 展示完整 provider page。研究完成后的 Owner 澄清还决定：Framed overlay 默认透明，只允许最小 contrast treatment，不保留大面积不透明 header card。具体 protected geometry 是否缩小仍由后续 preset design 和 visual fixtures 决定。
