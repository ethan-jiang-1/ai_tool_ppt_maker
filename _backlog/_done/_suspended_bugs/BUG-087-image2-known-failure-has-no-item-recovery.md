...
> 严重级别: P2 | 发现: 2026-08-19 | 状态: 悬挂 | 角色: bug | triage: wontfix
...
exit 0 是成功记录 known_failure，不是假装出图。批次未结束时 generate 下一项符合 progressive owner。全失败后的断裂由 BUG-088 修。不改 exit 语义。
