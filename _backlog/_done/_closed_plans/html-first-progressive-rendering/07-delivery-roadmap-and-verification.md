...
- legacy 显式迁移到 clean vNext：Agent 重写结构化 body、始终展示完整 proposed HTML deck/contact sheet；候选只在 `_scratch/html-migration/projected-run/` 通过 renderer-issued `migration-preview` context 渲染，scratch evidence 不可满足 canonical gates/build/state；旧侧使用 exact `verified-current|degraded-missing|degraded-stale` mode，只有 verified-current 展示旧 pixels，degraded 显示 diagnosis/placeholder 且不声称 parity；用户确认 exact mode/hash；apply 不复制 scratch generated bytes，必须 fresh canonical hidden-target rerender exact-match，且 target reviews 仍 pending；不自动猜 prompt，不在 migration 内触发 provider 或继承 gate authorization。
...
| 3D New-deck default | init/template/source 写 HTML-first marker，按 pipeline marker 分支 content/visual gate evidence与 preview/publication readiness；一个 deep review-evidence module 统一 read-only inspect、显式 journal recover、gate publication、delivery publication、state-first idempotent canonical reset，供 approve/checkBundle/state/status/refresh/Stage4/controllers/tests 共用；移除基础 Image2 onboarding，更新 intake/status/readiness/change classifier | 修改 `run-bundle-management`、`pipeline-orchestration`、`node-specification`、`cli-surface`、`framework-charter`、`commands-reference`、`bootstrap-env-guidance`、`playbook-execution` |
| 3E Compatibility/migration | legacy 路径保持；显式 clean vNext 迁移、对照 gate、零远端结构/重排 rebuild | 修改 `pipeline-orchestration`、`run-bundle-management`、`slide-identity-and-ordering`、相关 playbook specs |
...
- HTML-first content/visual gates 使用真实 content projection/HTML preview 且不要求 style master；`_state` 的 version-scoped evidence 是权威，metadata 只镜像。所有 canonical review plan/gate/delivery record 与 current manifest/receipt 绑定 state-owned nullable `html_production_reset_id`；plan hash 包含 reset ID，而 raw bytes 和 composition/final-slide/delivery identity 排除它。visual freshness 用 disjoint projections：global visual config/runtime/renderer/family registry/recipe/compositor versions 进入 system fingerprint；每个 current `component_recipe_key_v1` 必须有代表页；chart numeric shape 与 fallback/selection/inline-icon/placement 等 page-local visual dependency 进入排除 global/ordinary-copy/notes/position 的 `page_reviews[slide_id]` fingerprint。新 uncovered recipe key 需要 deck coverage review，已覆盖 key 或 page-local shape/bytes/resolution 变化只刷新受影响页。普通文案不使 visual gate stale；accepted selection fallback review 强制使用 identity 独立的 forced-fallback variant；legacy evidence 双向隔离。
- `state --check-gates` 先分类 pipeline：HTML 仅以 current reset ID、authoritative content/visual evidence、完整 approvable audit 和 fresh fingerprints 通过，journal/reset 冲突或异常 fail closed；HTML 使用独立 `html_*` + run-version mirrors 且不覆盖 legacy `content_gate|visual_gate` / `_state.gates.content|visual`，任何 mirror 不可让 HTML 通过，markerless 忽略 `html_*` 并保留自己的 scalar compatibility。plain state/status 只观察 journal/reset；state/status 显示 HTML outstanding recipe/page coverage、reset status 与 delivery-review freshness，历史 markerless 无 state 时只读不落盘，完整 HTML 不制造 Phase-4 欠账。
- approve/checkBundle/state/status/refresh/Stage 4/controller completion 通过同一个 review-evidence deep module 检查 freshness 或执行 reset；plain observation 是 read-only，build/check-gates/gate publication 才显式恢复 journal，只有 `refresh --kind reset-html-production --confirm-run-version <vN>` 可进入 state-first `deletion_pending -> complete` whole-owner reset。全部 HTML state writes 使用 expected-state CAS 并在 rename 前重查 reset fence；reset/journal creation 竞争只接受 exact reset-pending takeover projection 清理未提交 journal，其余第三 SHA fail closed。reset record 分离 semantic reset ID 与执行 owner claim：live owner 阻断，同机 dead 60 秒后单一 CAS claimant 自动接管，跨机/PID-uncertain 5 分钟后仅 human-confirmed 接管；崩溃重跑复用同一 reset ID，删除失败保持 fence。generated owner 若在已有 current-reset gate/delivery/Stage-4/5 authority 后意外丢失，直接重建必须先旋转新 ID；无 current authority 的首次/未批准 preview owner 缺失才可普通重建，byte-identical output 也不能复活旧 review。不存在各调用方自带 manifest 路径、digest、reset/owner ID、删除序列或 metadata gate 判断的平行实现。
...
验收还必须包含两条跨路径恢复：confirmed canonical HTML reset 必须先旋转 version reset ID/形成 `deletion_pending` fence，再完整删除 owner 并可零远端重建；状态写后、mirror 写后、删除中、完成写前任一点崩溃都复用同一 reset ID 恢复，且字节相同也不能复活旧 gate/final review。进程在 Image2 `submitting` 或 promotion commit 前后崩溃时，恢复不得重复计费或产生 source 指向不存在 asset 的半提交。
