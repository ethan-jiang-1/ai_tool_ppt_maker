...
- **用录下来的 trace 当"地面真相"**：`*.image2-task.json` 里录着**成功跑通那次**的真实请求/响应（submit/poll/`final_task_response`、base_urls）。据此**恢复**了缺失的 base_url、看清了"完成响应里 `data.result.images[0].url` 就是图"——把猜测变成证据。
...
- 改 image-API 解析/端点前，**先读 `*.image2-task.json` trace** 拿真实契约（submit/poll/result 形态、端点路径）。
