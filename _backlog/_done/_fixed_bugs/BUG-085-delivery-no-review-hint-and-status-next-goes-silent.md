...
> 严重级别: P2 | 发现: 2026-08-19 | 状态: 已修复 | 角色: bug | triage: fixed（范围收窄）
...
Maintainer 确认只修 `status` Next 哑火：消费已有 `workflow_inspection.primary_action`。不改 `build` 文案。吸收进 `restore-draft-and-cli-projections`。
