...
> Severity: P3 | Found: 2026-08-19 | Status: fixed | Role: bug | triage: fixed
...
Maintainer 确认。`style-master inspect` 必须声明 `--json`，与现有 CLI success report 契约对齐。默认仍 JSON。吸收进 `restore-draft-and-cli-projections`。
