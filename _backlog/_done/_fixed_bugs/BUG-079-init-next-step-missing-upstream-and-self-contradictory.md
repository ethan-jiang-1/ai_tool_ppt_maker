...
> 严重级别: P1 | 发现: 2026-08-19 | 状态: 已修复 | 角色: bug | triage: fixed（范围收窄）
...
Maintainer 确认只修「两条 Next 互相矛盾」：与 `ppt_flow init` 已有句子对齐（`status <v1Path>`）。「先喂上游 / 进度地图」随 Q1 拒绝。吸收进 `restore-draft-and-cli-projections`。
