...
- artifact 以 `(slide_id, render.engine, generation_fingerprint)` 寻址
- build 按当前 position 排序，再按 resolved engine 选取该 ID 的产物

这意味着仅重排页面时，两路已有产物都应复用；不能因为 `07 -> 03` 就重新跑 Image2 或 HTML。
...
3. sequential / parallel / per-slide override 只决定生成与选择哪种 artifact，不改变页面身份或顺序模型。
