...
or publishes `header_locked/`. A resolved `page-authority-image2-v1` /
`image2-page-authority` source/state pair SHALL be rejected before a Header-Lock call, slide-plan input,
or `header_locked/` publication is assembled. Page Authority publishes only through `finalizePage(...)`.
...
- **WHEN** a managed run resolves to the exact Page Authority source/state pair and a caller attempts a
  Header-Lock operation or historical Stage 3 adapter selection
- **THEN** the route returns the Page Authority finalization/recovery diagnostic before reading historical
