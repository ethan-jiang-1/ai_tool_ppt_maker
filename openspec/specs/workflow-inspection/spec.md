...
`inspectWorkflow` SHALL call the direct legacy protocol observer before selecting HTML, whole-page, or
Page Authority workflow prerequisites. For `recognized-legacy`, inspection SHALL return a non-mutating
...
- **WHEN** workflow inspection observes an exact HTML-first or recognized Image2 source/state pair
