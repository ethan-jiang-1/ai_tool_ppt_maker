...
marker-probe result: the explicit whole-page `legacy` branch, normalized elsewhere as pipeline
`whole-page-image2-v1`, SHALL become `image2-only`, and the explicit `html-first-v1` marker SHALL become
`html-only`. Migration SHALL NOT add `production.pipeline: whole-page-image2-v1` to explicit whole-page source.
...
#### Scenario: Historical explicit whole-page state is migrated

- **WHEN** a visible explicit whole-page version first enters the state-schema migration path
...
explicit whole-page maintenance without a first-class create execution retains its accepted compatibility
...
pipeline-owned review evidence. Historical explicit whole-page decks MAY lack `_state/` during structure-only
...
#### Scenario: Historical explicit whole-page deck lacks state

- **WHEN** structure-only check/status inspects an old explicit whole-page deck without `_state/`
...
including unambiguous schema-v3 migration. A historical explicit whole-page deck with no `_state/state.yaml`
...
For explicit whole-page legacy, `--check-gates` SHALL retain the existing scalar `isGateApproved` compatibility
...
current decision. Explicit whole-page compatibility output SHALL identify legacy-maintenance ownership without
...
#### Scenario: Historical explicit whole-page state inspection is non-writing

- **WHEN** a explicit whole-page historical deck has no `_state/state.yaml` and Agent runs state, status, or check-gates
...
#### Scenario: Missing historical explicit whole-page state is observed

- **WHEN** `readState` is called with `purpose: observe` for a explicit whole-page deck with no state
- **THEN** it returns compatibility status without creating `_state`

#### Scenario: Explicit legacy execution may recover state

- **WHEN** unambiguous explicit whole-page controller execution reads a broken legacy state in execute mode
...
freshness; they add only matching version identity. Explicit whole-page gate values/evidence retain whole-page
...
nullable reset ID null and shall not fabricate a reset record. A historical in-progress explicit whole-page
...
mode state, `production-mode-transition prepare` and `preview` SHALL not create or refresh a legacy candidate.  The only
...
#### Scenario: V2 explicit whole-page production becomes legacy maintenance

- **WHEN** an in-progress explicit whole-page deck points to an old whole-page production node
...
- **WHEN** schema-v4 explicit whole-page state has an active `apply-html-migration` record with a matching execution ID, `migration_source_version: v1`, plan hash, and old-side mode while multiple versions are visible
...
- **WHEN** schema-v4 explicit whole-page state is at `confirm-html-migration` with a matching completed preview and the closed confirmation re-inspects selected `v1`, hash, and mode
...
refinement record. Historical explicit whole-page maintenance SHALL retain its accepted compatibility completion
...
mode/pipeline entry conditions, and ensure no reserved ID is declared as a controller node. Image Production nodes require an explicit adapter role: `visual-slot` belongs to `image2-refine` and requires `html-then-image2` plus current delivery, while `whole-page` belongs to `image2-only` create work without HTML delivery. `create-deck` remains compatibility-only and rejects both HTML modes and fresh first-class Image2 ownership.
...
- **WHEN** a explicit whole-page historical run has no durable state
