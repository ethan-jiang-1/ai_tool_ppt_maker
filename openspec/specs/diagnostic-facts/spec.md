# diagnostic-facts Specification

## Purpose

Define the internal source/config problem-fact contract shared by the Page
Image source/configuration producer families (Page Source, Visual Language,
Presentation, Reference Material), their aggregators, and the operation owners
that consume their facts. It is the single authority for the minimal
cross-module problem-fact shape and preservation rules; the public
`pptmaker-cli-diagnostic` envelope and its projection rules remain owned by
`cli-surface`, and MD consumption by `node-specification`.
