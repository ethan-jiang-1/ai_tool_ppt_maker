...
`state` SHALL additionally expose only the controller-owned migration evidence operation `--confirm-migration-apply` with exact `--plan-hash` and `--old-side-mode` options. It is not a fourth `migrate-html` operation, a generic state editor, or a user-facing request to modify state.
