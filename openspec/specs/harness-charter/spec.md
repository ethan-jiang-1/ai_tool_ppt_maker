...
The guidance SHALL distinguish both from credentials, endpoint URLs, Page
Design System, presentation profiles, State, inspection, authorization, and
provider evidence.
...
action rather than recreate profile classification or a repair table. Live
smoke/probe evidence remains connectivity-only and SHALL not confirm a profile,
route, model, budget, unit, prompt fit, authorization, or future provider work.

This guidance adds no Task Mandate, provider authorization, human cost
confirmation, Controller branch, State record, provider probe, fallback, or
...
- **AND** it does not infer facts from a credential, endpoint, model alias,
  smoke result, prior plan, or provider failure
...
- **THEN** it distinguishes the environment-owned runtime profile ID from the
  source declaration and treats live success as connectivity-only
