...
- **AND** it does not invoke a retired or sibling workflow adapter

## ADDED Requirements

### Requirement: Image production resolves only selected v2 workflow owners
Image Production SHALL resolve an exact v2 source/state pair marker-first to the selected `03-framed-image` or `04-pure-image` adapter followed by shared `05-delivery`. A retired, partial, hybrid, or mismatched pair SHALL not resolve an adapter, writer, receipt initializer, or finalization path; it receives the owning migration/export or identity hard-stop.
...
- **AND** no compatibility or retired adapter is exported, registered, or imported
