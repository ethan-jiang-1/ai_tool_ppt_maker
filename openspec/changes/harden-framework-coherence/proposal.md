...
- Reconcile the gate-condition catalog with every active playbook, distinguishing deterministic conditions from explicitly manual human-judgment conditions.
...
- Add automated coherence checks for Markdown links, forbidden/stale production paths, edit-chain semantics, playbook node schemas, node uniqueness, condition resolution, and standalone CLI error envelopes.
...
- Affected runtime code: `PPTMAKER_FRAMEWORK/scripts/lib/state.mjs`, standalone CLI entry points under `PPTMAKER_FRAMEWORK/scripts/`, and potentially shared CLI error helpers.
- Affected controller content: `PPTMAKER_FRAMEWORK/playbook/*.md` and the gate-condition catalog in `charter/NODE-SPEC.md`.
