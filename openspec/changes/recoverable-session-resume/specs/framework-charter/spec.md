...
§1 入口顺序 (or equivalent entry iron law) SHALL require that for an existing `deck_*`, progress truth is `_state/state.yaml` (not conversation context): agents SHALL run the resume ritual (`ppt_flow state` / read state, continue at `current_node`) before greenfield intake, and SHALL persist node transitions with `writeState`. Cleared chat or a new session SHALL NOT be treated as lost progress when `_state` exists.
...
- **THEN** AGENT_CONTRACT directs the agent to treat `_state/state.yaml` as progress SSOT
