...
> Implementation status: baseline artifacts are being captured through focused architecture/interface tests; no baseline task is complete until the post-move parity assertions pass.
...
> Implementation status: source, test, and workflow Phase-4 trees have moved to `04-image-production`; public interfaces and import/ownership inventories are being repaired against the architecture suite before this section is checked off.
...
> Implementation status: the state owner now has the new-first/old-fallback reader and a CAS writer; application promotion and terminal-decline writers are next, followed by compatibility fixtures.
...
> Implementation status: pending until the relocated interfaces, state migration, and focused suites are green.
