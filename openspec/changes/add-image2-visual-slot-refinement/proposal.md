...
现在 Change 4 已提供独立的 README-only Phase 4 ownership、受约束接口和测试树，可在不改变 HTML 成品默认路径的前提下引入这条可选的付费能力。
...
- `cli-surface`: 定义 `ppt_flow image2` 及 Phase-4 direct CLI 的封闭命令、失败回执和成本安全边界。
- `commands-reference`: 路由可选主视觉精修，不把它呈现为新 deck 的必经 renderer 选择。
- `framework-charter`: 记录人类对远端成本和逐页视觉采用的控制权，以及现代/legacy Image2 的隔离。
- `framework-script-layout`: 将 Phase 4 从 README-only 保留位升级为受限 public interface、direct CLI 与镜像测试 owner。
