...
- [ ] 1.1 [content-parsing] Extend the canonical production-marker parser for the closed `page-authority-image2-v1` mapping, then implement the per-slide authority resolver, typed Text Frame/visual-brief receipt, and source-span diagnostics; reject legacy prompt/render ingress for this marker.
...
- [ ] 2.1 [node-specification] Extend the sole `production_mode.mjs` policy table and state validation with `image2-page-authority` / `page-authority-image2-v1`; add source epoch ownership, authorization scope, raw-review references, and bounded repair diagnostics without copying policy in CLI or Controller code.
...
- [ ] 5.4 [workflow-inspection] Extend the controller-reader vocabulary and the sole read-only projection for the Page Authority pipeline; hash and order only direct Page Authority prerequisites and prove it never substitutes HTML review, Image2 refinement, or Header-Lock for the exact source/state pair.
- [ ] 5.5 [commands-reference, bootstrap-env-guidance] Rewrite new-deck and BOOTSTRAP guidance around Pure/Framed ownership, unbound Page Authority readiness, and later operation-scoped credentials; remove legacy production choices from new-deck routing while retaining them only for explicitly targeted existing runs.
- [ ] 5.6 [tests] Add CLI/Controller/doctor/BOOTSTRAP/inspection negative coverage for unauthorized work, invalid identity, missing review coverage, runtime-only refresh without provider credentials, legacy-stage exclusion, and bounded recovery diagnostics.
