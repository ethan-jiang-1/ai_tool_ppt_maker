...
**Reason**: The v2 local frame incorrectly owns callouts and supports a
text-free provider underlay.

**Migration**: New source uses the closed Provider Content Schema; no v2 source
or receipt is converted automatically.

### Requirement: Page Authority source admits only a closed visual-brief selection

**Reason**: Its Page Authority naming and authority/text constraints describe
the retired v2 source model.

**Migration**: Preserve the closed registered visual selection under the
separate current Page Image visual-language and Provider Content Schema
boundaries.

### Requirement: Current source parsing is Page Authority-only

**Reason**: v2 is no longer a current production grammar.

**Migration**: Current source uses `page-image-workflow-v1`; v2 hard-stops.

### Requirement: TARGET Page Authority source selects one version workflow

**Reason**: The replacement protocol has its own identity and receipt schema.

**Migration**: Re-author the source under `page-image-workflow-v1`; do not
reuse a v2 receipt.

### Requirement: VISUAL SCENE is an optional single-occurrence inline source field

**Reason**: The v2 grammar and no-text interpretation are retired.

**Migration**: The replacement grammar retains visual direction only through
its closed current fields and receipt.

### Requirement: BODY is an optional single-occurrence inline source field

**Reason**: A free inline body cannot represent provider-visible content
authority or literal policy.

**Migration**: Express visible provider copy through `SLIDE BODY.items`; no
inline body is upgraded.
