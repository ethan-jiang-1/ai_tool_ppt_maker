...
- Introduce a `workflow-inspection` capability with a versioned, read-only workflow projection over existing direct owners. It reports the exact checkpoint, `guide|confirm|hard-stop` posture, earliest bounded root cause, exactly one ordered `primary_action`, non-primary observations, optional allowed continuation, protected invariant, and attributable evidence summary.
- Make `status --json` and `state --json` expose the same byte-equivalent nested `workflow_inspection` projection while preserving all existing raw durable-state and compatible status fields. Human-readable status/state and resume guidance consume the same primary action instead of independently deriving one.
