...
adapter may dispatch to that compatibility interface only after exact v1
routing. Target adapters, shared workflow inspection, and controller
observation must not import or invoke it.
...
Static import checks will reject a target adapter, shared observer, or
controller-observation path that imports the compatibility mutation surface.
The top-level exact-route dispatcher is the narrow explicit exception.
...
writer edge. The 05-delivery owner will continue to validate both compatible
final-manifest provenances through one delivery interface rather than branch on
workflow semantics.
