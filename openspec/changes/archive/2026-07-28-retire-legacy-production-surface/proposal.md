...
- Keep shared current command names such as build, refresh, status, and state only for Page Authority;
  an intact historical run reaching one of them receives the existing adoption diagnostic. Remove
  legacy-only commands, flags, help entries, and approval choices rather than keeping a hidden route.
...
- Encode every non-`Keep` ledger row as an exact-title delta removal with an explicit migration owner;
  the current Page Authority requirement in the same capability is the replacement contract. `Keep`
  rows remain in their accepted main spec and receive a named focused regression obligation.
