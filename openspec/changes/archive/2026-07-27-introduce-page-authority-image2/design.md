...
The state owner initializes each Page Authority version's `source_epoch` at `1` and advances it only when
raw-source authority changes. A provider-profile change invalidates raw reuse/review but does not advance
that epoch. Page Authority finalization consumes
only a current human `proceed` record covering the exact raw tuple. Its final manifest, projection,
PPTX, and notes receipts are then derived in order. A changed Framed Text Frame can retain raw coverage only when all
raw and review identity facts remain exact; it still creates new final/delivery evidence.

Structural editing keeps the existing preview -> exact plan hash -> clean vNext transaction. For a
Page Authority target, preview classifies each retained slide as an exact raw tuple eligible for
byte-verified materialization or `needs_raw_generation`; that classification and the source lineage are
covered by the plan hash. Apply initializes the target's Page Authority `source_epoch` at `1`, writes
only target-owned `unreviewed` raw provenance for verified materializations, and records the remaining
raw debt. It copies no raw-review coverage, final/projection/PPTX/notes/delivery evidence, or provider
authorization, and it makes no provider call. The target requires a version-scoped human raw acceptance
...
probe remains explicit. Invalid source/state, missing/partial/stale raw evidence, invalid provider scope,
unknown submission, or an attempted provider call without recorded authorization are hard-stops with one
owner-issued recovery action. A complete current raw/final projection awaiting `proceed|repair|redirect`,
or a complete disclosed raw-submit scope awaiting authorization, is a human `confirm` gate; neither
decision is inferred from a prior batch, chat, or receipt.
