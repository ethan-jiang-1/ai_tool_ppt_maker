...
- **Shared key path:** non-empty `IMAGE2_API_KEY`, or legacy aliases `IMAGE2_API_KEY` / `IMAGE2_API_KEY`.
- **Vendor-list path:** when `IMAGE2_BASE_URL` is non-empty, every listed item MUST resolve a key (`process.env[KEY_ENV]` or shared-key fallback for items without `|KEY_ENV`). If any item cannot resolve a key, `api_key` SHALL fail and fix text SHALL name the missing variable(s).

When neither a shared key nor a fully resolvable `IMAGE2_BASE_URL` key set is available, `api_key` SHALL fail. Fix text SHALL name `IMAGE2_API_KEY` and MAY also name `IMAGE2_BASE_URL` / the missing `KEY_ENV`.
...
- **AND** `IMAGE2_BASE_URL` is unset
- **THEN** `api_key` still passes

#### Scenario: IMAGE2_BASE_URL keys satisfy api_key without shared IMAGE2_API_KEY

- **WHEN** `IMAGE2_BASE_URL` lists `url|KEY_ENV` items whose KEY_ENV values are all set
- **AND** `IMAGE2_API_KEY` / OPENAI / IMAGE2 shared keys are unset
- **THEN** `api_key` passes

#### Scenario: IMAGE2_BASE_URL with missing KEY_ENV fails api_key

- **WHEN** `IMAGE2_BASE_URL` lists `https://example/v1|MISSING_KEY_VAR` and `MISSING_KEY_VAR` is unset
...
- **WHEN** no shared Image2 key is set and `IMAGE2_BASE_URL` is unset/empty
...
- non-empty `IMAGE2_BASE_URL` (ordered `base_url|KEY_ENV` list), **or**
- non-empty `IMAGE2_BASE_URL` or `IMAGE2_BASE_URL`, or legacy `IMAGE2_BASE_URL` / `IMAGE2_BASE_URL` / `IMAGE2_BASE_URL`.

When none is set, `image_base_url` SHALL be **`fail`** and overall NOT READY. The check SHALL NOT claim a silent default endpoint when URL is unset. Fix text SHALL name `IMAGE2_BASE_URL` and MAY also name `IMAGE2_BASE_URL`.
...
#### Scenario: IMAGE2_BASE_URL alone satisfies the check

- **WHEN** `.env` has non-empty `IMAGE2_BASE_URL` with at least one parseable `base_url` item
- **AND** no `IMAGE2_BASE_URL` / `IMAGE2_BASE_URL`
- **THEN** `image_base_url` passes

#### Scenario: Missing base URL fails doctor

- **WHEN** a shared key is present but no base URL variable and no `IMAGE2_BASE_URL` is set
...
- **AND** a suggested `IMAGE2_BASE_URL` ordering appears
