...
- Isolate the retained exact CURRENT v1 Page Authority implementation, guidance, and focused proof under an explicit compatibility ownership surface. TARGET code, controller/status observation, and every route resolved to a non-v1 pair will not import or invoke a v1 writer; the exact-v1 mutation dispatcher remains its sole explicit caller.
