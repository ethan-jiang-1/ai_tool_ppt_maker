...
The framework directory map and executable inventory SHALL give every
registered production executable one declared current owner, and SHALL expose
the retained CURRENT v1 Page Authority implementation only through the
explicit compatibility/current-v1-page-authority ownership surface. Its source,
workflow guidance, and focused proof SHALL respectively live under the
scripts, workflow, and tests domain roots using that same logical path. The map
SHALL distinguish that existing-run-only compatibility owner from the target
method graph and SHALL NOT expose retired production owners, generic
...
- **THEN** every registered production executable has one declared current owner
- **AND** every Page Authority implementation belongs either to its selected
  target workflow owner or the explicit CURRENT v1 compatibility owner
