...
- A real run bundle such as `deck_ai_sdlc_bpm_keynote` is produced by `bundle_layout.mjs#initBundle`. The producer currently seeds `CLAUDE.md` plus `deck-guide.md`, but no root `AGENTS.md`; agent-agnostic runtime discovery must therefore be fixed in the scaffold producer, not by hand-editing one generated deck.
...
6. Add repository-maintenance pointers and producer-owned run-bundle controls/templates. Verify only a fresh temporary scaffold; do not edit `deck_ai_sdlc_bpm_keynote` or placement-map README seeds.
