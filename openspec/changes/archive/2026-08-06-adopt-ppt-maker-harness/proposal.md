...
- **BREAKING** Replace the canonical Harness root `PPTMAKER_FRAMEWORK/` with
  ...
  name throughout active source, CLI guidance, package metadata, and normative
  OpenSpec specifications. Focused negative test fixtures MAY retain literal
  legacy values only to prove rejection; they SHALL not exercise an accepted
  compatibility route.
...
- **BREAKING** Make fresh Run Bundles bind to their creating local Harness via a
  ...
  if it is ever to be used again. A structure-only layout inspection remains a
  non-authoritative, read-only exception and cannot resume or operate a Bundle.
...
accepts only v2 Harness-bound bundles for run-scoped operations. Existing
`deck_*` and `dpt_*` production data are not source, fixtures, or automatic
migration targets, and they will not be read or modified by this change. A
structure-only layout inspection may report an old Bundle without making it a
current input, selecting a run, or authorizing a write. Archived OpenSpec
changes are also out of scope; active `openspec/specs/` is the normative
contract to be updated.

Focused temporary-directory tests MAY construct legacy locator bytes solely to
prove the hard-stop before state or production work. Those fixtures neither
restore v1 parsing as an accepted protocol nor authorize an alias, migration,
or fallback root.
