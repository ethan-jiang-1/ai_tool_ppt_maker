...
When selected-workflow visual/source drift has made an accepted Style Master
selection stale but the Style Master owner has published one valid current
successor plan whose immutable predecessor identity matches that stale
selection, the view SHALL consume the `style-master-generation` owner's
read-only pending-successor candidate projection and project that plan as
pending, not accepted, Style Master work. It SHALL list an available
local-existing or succeeded generated candidate only when that owner provides
verified immutable media and a confined locator. For each other generated slot,
the view SHALL identify its stable candidate ID and owner-established lifecycle
state as unavailable without a locator. The prior selection remains historical
audit evidence and SHALL not be displayed as current Style Master authority.
This pre-selection view SHALL not publish a Page Image source epoch, raw plan,
authorization, provider attempt, review, accepted raw evidence, final media,
PPTX, notes, or delivery as available. It SHALL stop before raw-owner,
stored-raw-plan, or raw-only accepted-selection inspection.
...
#### Scenario: A successor plan exposes only its available candidate media

- **WHEN** a valid current Style Master successor plan has one verified
  local-existing candidate and two planned generated slots after its prior
  selection became stale, and its predecessor identity matches that selection
- **THEN** the rebuilt view lists the local candidate as pending with its
  owner-issued locator and identifies both generated slots as unavailable by
  stable ID and planned lifecycle state
- **AND** it does not treat the stale selection, planned slots, raw work, or
  later delivery artifacts as current available evidence
