...
`PPTMAKER_FRAMEWORK/scripts/fonts/` SHALL be the canonical soft-bundle root for framework-distributed font binaries used by the HTML runtime, their generated local CSS/coverage manifest, provenance, copyright notices, and license texts. The tree MAY contain family-specific and license subdirectories beneath `scripts/fonts/`; it SHALL NOT add a sixth top-level `PPTMAKER_FRAMEWORK/` directory and SHALL NOT define or write font assets inside a `deck_*` run bundle.
...
- **THEN** the binaries, manifest/CSS, provenance, and license files are all discoverable under `PPTMAKER_FRAMEWORK/scripts/fonts/`
