...
| `requestedFrameworkRoot`, declared/relation/requested selection | no caller-selected fallback; declared and relation roots must agree |
...
wrong-sentinel locator fails before state, provider, generated-artifact, or
production work. It does not heal or rewrite the manifest. The diagnostic gives
the one nearest action: explicitly reconstruct a current Bundle if the user
wants to resume that content later.
...
diagnostic labels. The v2 locator remains the only MD-to-JS binding protocol;
...
- **A broad rename misses an active command or import** -> Use a scoped
  ...
  audit permits only deliberate historical citations outside the active runtime
  surface.
...
3. Replace the locator implementation and all consumers with the v2 API:
   ...
   v2 manifest; emit the existing bounded CLI error envelope for unsupported
   bindings.
4. Update all current CLI, MD guidance, help text, inventories, architecture
   checks, fixtures, and tests to the Harness root. Rename every active
   Framework-named schema to a Harness v2 schema, including the governance
   ledger fixture and contract test.
5. Add focused tests for v2 rendering/resolution, direct/relation mismatch,
   v1 rejection with zero writes, absence of fallback root selection, and
   Bundle-outside-Harness enforcement. Update existing unit and e2e command
   paths rather than introducing production data fixtures.
6. Validate in layers: unit tests cover pure locator/schema behavior;
   integration tests cover `init`, `--check`, and bounded CLI diagnostics in
   temporary directories; e2e tests cover fresh Harness initialization and
   command discovery without provider work. Run the full test suite, OpenSpec
   validation, and the scoped active-surface audit last.
