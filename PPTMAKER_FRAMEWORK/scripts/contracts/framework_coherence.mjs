...
/** The sole catalogue of temporary retired-token compatibility. Entries are
 * deliberately literal: a path or wildcard cannot silently widen a legacy
 * spelling into a repository-wide exception. */
...
export function extractNodeCommands(file, text = readFileSync(file, "utf8")) {
  ...
  const inline = /`([^`\n]*\bnode\s+(?:PPTMAKER_FRAMEWORK\/)?scripts\/[^`\n]+)`/g;
...
export function validatePseudocodeMarkers(file, text = readFileSync(file, "utf8")) {
  ...
  for (const match of text.matchAll(/<!--\s*coherence:pseudocode\b[\s\S]*?-->/g)) {
    ...
    const count = (example.match(/(?:^|\s)node\s+(?:PPTMAKER_FRAMEWORK\/)?scripts\//gm) || []).length;
...
export function validateDiagnosticAuthorityPointers({ root = "." } = {}) {
  ...
  const requirements = [
    ...
    ["PPTMAKER_FRAMEWORK/scripts/shared/cli/cli_error.mjs", ["openspec/specs/cli-surface/spec.md"]],
    ["PPTMAKER_FRAMEWORK/scripts/shared/state/md_controller_reader.mjs", ["openspec/specs/node-specification/spec.md"]],
    ["PPTMAKER_FRAMEWORK/scripts/shared/state/state.mjs", ["openspec/specs/node-specification/spec.md"]],
  ];
