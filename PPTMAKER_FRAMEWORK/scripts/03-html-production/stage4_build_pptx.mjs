...
import { sha256File } from "../05-iteration/legacy-image2/internal/image_provenance.mjs";
import { HTML_FIRST_PIPELINE, validateAndBuildHtmlFirstPlan } from "./internal/html_slide_contract.mjs";
import { inspectHtmlReviewReadiness } from "../shared/state/html_review_evidence.mjs";
import { publishHtmlDeliveryContactSheet } from "./internal/html_preview.mjs";
...
export async function buildPptx({ images, slidePlan, out, title = "Presentation" }) {
  ...
  if (basename(planGenerated) === '_generated' && existsSync(candidateSource) && (await import('./internal/html_slide_contract.mjs')).probeProductionMarker(readFileSync(candidateSource)).branch === HTML_FIRST_PIPELINE) throw new Error('USAGE: legacy artifact mode cannot target an HTML-first run; use --run-dir');
...
export async function buildPptxFromRunDir(runDir, title = "Presentation") {
  ...
  if (existsSync(sourcePath)) {
    try {
      const marker = (await import('./internal/html_slide_contract.mjs')).probeProductionMarker(readFileSync(sourcePath));
