...
import { COLOR_PALETTE_FILE } from "../../../shared/run-bundle/bundle_layout.mjs";
import {
...
async function runLockHeaders(opts) {
  ...
  for (let i = 0; i < slides.length; i++) {
    ...
    const headerFingerprint = sha256Bytes(canonicalJson(headerProfile));
    const finalEntry = {
      slide_id: slideId,
...
export async function runHeaderLockCli(argv = process.argv) {
  let opts;
  try {
    ...
    emitCliError({
      ...
      diagnostic: structured || {
        ...
        next: createCliNext("repair_prerequisite", {
          ...
          invocation: opts.images && opts.slidePlan && opts.out ? { program: "node", args: [HEADER_LOCK_CLI, "--images", opts.images, "--slide-plan", opts.slidePlan, "--out", opts.out] } : undefined,
