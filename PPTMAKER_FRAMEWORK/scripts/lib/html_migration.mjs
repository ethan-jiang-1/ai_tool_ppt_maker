...
import { checkBundle, deckRoot, findSlideSpecs, nextVersionName, SCRATCH_SUBDIR, GEN_QA_SUBDIR, checkStagedVersion } from '../bundle_layout.mjs';
import { probeProductionMarker, validateAndBuildHtmlFirstPlan } from './html_slide_contract.mjs';
import { createCanonicalHtmlValidatedRunContext, createMigrationPreviewHtmlValidatedRunContext, publishHtmlComposition } from './html_slide_renderer.mjs';
import { classifyHtmlOwnerLiveness, htmlOwnerRoot, readHtmlCurrentManifest, readHtmlPreviewManifest } from './html_object_store.mjs';
import { publishHtmlDeliveryContactSheet } from './html_preview.mjs';
import { readState } from './state.mjs';
import { ARTIFACT_KIND_FINAL_SLIDE, ARTIFACT_STATUS_VERIFIED, RENDER_ENGINE_IMAGE2, readArtifactManifest, resolveRenderArtifact } from './render_artifacts.mjs';
...
async function renderCanonicalTarget(sourceRunDir, targetVersion, token, expectedPlanHash) {
  ...
  try {
    ...
    const stage1 = await import('../unified_pipeline.mjs').then((mod) => mod.stage1);
