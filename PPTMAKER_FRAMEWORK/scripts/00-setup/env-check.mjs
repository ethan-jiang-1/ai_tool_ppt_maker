...
import { HTML_RUNTIME_PROFILE } from './internal/html_runtime_profile.mjs';
...
async function checkHtmlRuntime(playwright) {
  const { inspectHtmlRuntime, runHtmlRuntimeSmoke } = await import('./internal/html_runtime.mjs');
  const { verifyHtmlFontBundle } = await import('./internal/html_fonts.mjs');
...
async function runAllChecks({ includeImage2 = false, start = process.cwd() } = {}) {
  ...
  if (includeImage2) {
    ...
    if (apiKey.status === 'ok' && baseUrl.status === 'ok' && stage2.status === 'ok') {
      try {
        const { resolveVendors } = await import('../05-iteration/legacy-image2/internal/image_api_client.mjs');
...
async function checkImageSmoke({ vendors: injectedVendors, fetchImpl = globalThis.fetch } = {}) {
  try {
    const {
      ...
      DEFAULT_MODEL,
      providerHost,
    } = await import('../05-iteration/legacy-image2/internal/image_api_client.mjs');
