 ...
 *
 * Vocabulary note: `legacy-image2-first` is the NORMALIZED name for the
 * canonical markerless whole-page branch. It is never written as source
 * frontmatter; the markerless branch carries no `production.pipeline` marker at
 * all. Treating it as frontmatter is a category error — use pipelineFromSourceMarker.
 */
import { HTML_FIRST_PIPELINE, LEGACY_PIPELINE } from "./production_marker.mjs";
 ...
 *
 * The image2-only pipeline value is the normalized markerless-branch label, not
 * source frontmatter.
 *
 * @readonly
 */
...
/**
 * Normalize a marker probe result into a derived source pipeline. The markerless
 * `legacy` branch is normalized to `legacy-image2-first`; the explicit
 * `html-first-v1` branch stays `html-first-v1`. This is the ONLY sanctioned
 * interpretation of the markerless label.
 *
...
export function pipelineFromSourceMarker(sourceMarker) {
  ...
  if (branch === "legacy") {
    return { ok: true, pipeline: LEGACY_PIPELINE, branch };
  }
  return {
    ok: false,
...
/**
 * Closed migration mapping: marker probe -> authoritative mode. Explicit
 * `html-first-v1` becomes `html-only`; the markerless `legacy` branch becomes
 * `image2-only`. Invalid/unknown markers fail closed. This is the pure rule the
 * state migration consumes; it never consults refinement, metadata, history, or
 * generated bytes.
 *
 * @param {{ branch?: string, issues?: unknown[] } | null | undefined} sourceMarker
...
export function productionModeFromSourceMarker(sourceMarker) {
  ...
  return { ok: true, mode: "image2-only", pipeline: LEGACY_PIPELINE, branch: pipeline.branch };
}
