...
function directProductionNodeIssues(frontmatter, source) {
  ...
  if (pairs.length !== 1) {
    if (Object.hasOwn(frontmatter.metadata || {}, "production")) {
      issues.push(markerIssue("invalid_production_marker", "production must be one direct string-keyed mapping", { source }));
    }
    return issues;
  }
  if (pairs[0].key.anchor || pairs[0].key.tag) {
    issues.push(markerIssue("invalid_production_key", "production must use a direct untagged string key", { source }));
  }
  const production = pairs[0].value;
  if (!isMap(production) || production.anchor || production.tag) {
    issues.push(markerIssue("invalid_production_marker", "production must be a direct mapping", { source }));
    return issues;
  }
  for (const pair of production.items) {
    if (!isScalar(pair.key) || typeof pair.key.value !== "string" || pair.key.anchor || pair.key.tag) {
      issues.push(markerIssue("invalid_production_key", "production keys must be direct strings", { source }));
    } else if (pair.key.value !== "pipeline") {
      issues.push(markerIssue("unknown_production_key", `unknown production key ${JSON.stringify(pair.key.value)}`, { source }));
    }
  }
  ...
  if (
    ...
    issues.push(markerIssue("invalid_pipeline_marker", "production.pipeline must be one direct string scalar", { source }));
  }
  return issues;
}
