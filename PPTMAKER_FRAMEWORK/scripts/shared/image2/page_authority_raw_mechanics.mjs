...
import { inspectPageAuthorityRawProviderAuthorization } from "../state/state.mjs";

const SHA256_RE = /^[0-9a-f]{64}$/;

export class PageAuthorityRawMechanicsError extends Error {
  constructor(code, message) {
    super(message);
    this.name = "PageAuthorityRawMechanicsError";
    this.code = code;
  }
}

function checkedPlan(rawWorkPlan) {
  const checked = validateRawWorkPlan(rawWorkPlan);
  if (!checked.ok) throw new PageAuthorityRawMechanicsError(checked.code, checked.message);
  return checked;
}
...
function currentAuthorization({ deckDir, runVersion, runDir, rawWorkPlan }) {
  ...
  if (!authorization.ok) {
    throw new PageAuthorityRawMechanicsError("provider_authorization_required", `Page Authority provider authorization failed: ${authorization.code}`);
  }
...
export async function submitAuthorizedRawWorkPlan({ deckDir, runVersion, runDir, rawWorkPlan, submit } = {}) {
  ...
  if (typeof submit !== "function") throw new PageAuthorityRawMechanicsError("provider_submit_required", "a provider submit function is required");
...
export function publishAcceptedRawEvidence({ deckDir, runVersion, runDir, rawWorkPlan, raw_review_sha256, raw_bytes_by_slide } = {}) {
  ...
  if (!SHA256_RE.test(raw_review_sha256 || "")) {
    throw new PageAuthorityRawMechanicsError("raw_review_required", "a current raw review SHA-256 is required");
  }
  const authorization = currentAuthorization({ deckDir, runVersion, runDir, rawWorkPlan });
