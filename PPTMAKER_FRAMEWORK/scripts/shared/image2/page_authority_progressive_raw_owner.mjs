...
} from "./page_authority_progressive_store.mjs";
...
/** Build the only provider-free v3 raw plan from one selected adapter plan. */
export function createProgressiveRawWorkPlanFromTarget({ runDir, source_epoch, raw_work_plan, effective_style_master_sha256 } = {}) {
  const legacy = checked(raw_work_plan, "selected adapter raw work plan", (plan) => {
    if (!plan || plan.schema !== "page-authority-raw-work-plan-v2") return { ok: false, code: "progressive_raw_target_plan_invalid", message: "selected adapter must supply a current v2 typed raw plan projection" };
    ...
    return Array.isArray(ids) && ids.length > 0 && Array.isArray(plan.items) && ids.length === plan.items.length
      ...
      : { ok: false, code: "progressive_raw_target_plan_invalid", message: "selected adapter raw plan is incomplete" };
  });
  ...
  const plan = createProgressiveRawWorkPlan({
    ...
    source_execution_sha256: canonicalJsonSha256({
      schema: "page-authority-raw-source-execution-v3",
      run_version: basename(runDir || ""),
      source_receipt_sha256: raw_work_plan.source_receipt_sha256,
      source_epoch,
      workflow: raw_work_plan.workflow,
      provider_profile_sha256: raw_work_plan.provider_profile_sha256,
      effective_style_master_sha256,
      raw_work_plan_v2_sha256: legacy.sha256,
    }),
    ordered_slide_ids: raw_work_plan.ordered_slide_ids,
    items: raw_work_plan.items.map((item) => ({ slide_id: item.slide_id, raw_contract_sha256: item.raw_contract_sha256 })),
  });
  return plan;
}
...
export function inspectProgressiveRawLifecycle({ runDir, workflow, expected_plan = null } = {}) {
  ...
  if (!snapshot) {
    ...
    const legacy = existsSync(paths.target_raw_plan) || existsSync(paths.target_raw_evidence);
    return Object.freeze({
      ok: true,
      legacy_v2: legacy,
      plan: null,
      ...
      primary_action: legacy
        ? action("replan_progressive_raw_work", { kind: "repair", summary: "Legacy v2 raw records remain readable but cannot seed a progressive head." })
        : action("plan_progressive_raw_work", { summary: "Compile one provider-free full progressive raw plan." }),
    });
  }
...
export async function generateProgressiveRawItem({ runDir, workflow, plan_hash, batch_hash, expected_plan = null, provider_requests_by_slide, submit } = {}) {
  if (typeof submit !== "function") fail("progressive_raw_provider_submit_required", "one provider submit function is required");
  asObject(provider_requests_by_slide, "provider_requests_by_slide");
  return withCurrentProgressiveRawPlanLock(runDir, {
    workflow,
    plan_hash,
    expected_plan,
    action: async (snapshot) => {
      ...
      const request = provider_requests_by_slide[candidate.record.slide_id];
      if (!request || typeof request !== "object") fail("progressive_raw_provider_request_invalid", `the selected adapter request is unavailable for ${candidate.record.slide_id}`);
...
export async function reconcileProgressiveRawAttempt({ runDir, workflow, plan_hash, attempt_sha256, lookup = null } = {}) {
  ...
  return withProgressiveRawPlanLock(runDir, {
    ...
    action: async () => {
      ...
      if (!current) fail("progressive_raw_plan_missing", "historical reconciliation requires a current selected scope head");
...
export async function acceptProgressiveRawPilot({ runDir, workflow, plan_hash, batch_hash, decision } = {}) {
...
/** Decide a complete full-plan review and publish the sole accepted v3 evidence on proceed. */
export async function acceptProgressiveRawCompleteReview({ runDir, workflow, plan_hash, decision } = {}) {
  if (!["proceed", "repair", "redirect"].includes(decision)) fail("progressive_raw_complete_review_decision_invalid", "complete raw-review decision must be proceed, repair, or redirect");
  return withCurrentProgressiveRawPlanLock(runDir, {
    workflow,
    plan_hash,
    action: async (snapshot) => {
