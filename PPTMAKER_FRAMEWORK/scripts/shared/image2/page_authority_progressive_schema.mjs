...
export const PROGRESSIVE_RAW_WORK_PLAN_SCHEMA = "page-authority-raw-work-plan-v3";
export const PROGRESSIVE_RAW_SCOPE_HEAD_SCHEMA = "page-authority-raw-scope-head-v3";
export const PROGRESSIVE_RAW_BATCH_SCHEMA = "page-authority-raw-batch-projection-v3";
export const PROGRESSIVE_RAW_BATCH_GRANT_SCHEMA = "page-authority-raw-batch-grant-v3";
export const PROGRESSIVE_RAW_ITEM_ATTEMPT_SCHEMA = "page-authority-raw-item-attempt-v3";
export const PROGRESSIVE_RAW_MATERIALIZATION_PROVENANCE_SCHEMA = "page-authority-raw-materialization-provenance-v3";
export const PROGRESSIVE_RAW_PILOT_EVIDENCE_SCHEMA = "page-authority-raw-pilot-evidence-v3";
export const PROGRESSIVE_RAW_PILOT_DECISION_SCHEMA = "page-authority-raw-pilot-decision-v3";
export const PROGRESSIVE_RAW_COMPLETE_REVIEW_SCHEMA = "page-authority-raw-complete-review-v3";
export const PROGRESSIVE_ACCEPTED_RAW_EVIDENCE_SCHEMA = "page-authority-accepted-raw-evidence-v3";
...
export const PROGRESSIVE_RAW_REVIEW_DECISIONS = Object.freeze(["proceed", "repair", "redirect"]);

const SHA256_RE = /^[0-9a-f]{64}$/;
const RUN_VERSION_RE = /^v[1-9][0-9]*$/;
const SLIDE_ID_RE = /^[A-Za-z][A-Za-z0-9_-]{0,127}$/;
const IDEMPOTENCY_KEY_RE = /^page-authority-v3-[0-9a-f]{64}$/;
...
function assertItems(items, ids, { plan = null } = {}) {
  ...
  for (const item of items) {
    if (!exactKeys(item, ["slide_id", "raw_contract_sha256"])) {
      fail("progressive_raw_invalid_items", "each item must contain only slide_id and raw_contract_sha256");
    }
  ...
  if (plan) {
    const planItems = new Map(plan.items.map((item) => [item.slide_id, item.raw_contract_sha256]));
    for (const item of items) {
      if (planItems.get(item.slide_id) !== item.raw_contract_sha256) {
        fail("progressive_raw_cross_bound", "item raw contract does not match the referenced full plan");
      }
    }
  }
}
...
export function progressiveRawAttemptKey({ plan_sha256, batch_sha256, slide_id, raw_contract_sha256 } = {}) {
  ...
  return canonicalJsonSha256({ schema: "page-authority-raw-attempt-key-v3", plan_sha256, batch_sha256, slide_id, raw_contract_sha256 });
}

export function progressiveRawIdempotencyKey({ attempt_key_sha256 } = {}) {
  assertDigest(attempt_key_sha256, "attempt_key_sha256");
  return `page-authority-v3-${attempt_key_sha256}`;
}
...
export function validateProgressiveRawPilotDecision(decision, { plan = null, batch = null, evidence = null } = {}) {
  return result(() => {
    ...
    if (!PROGRESSIVE_RAW_REVIEW_DECISIONS.includes(decision.decision)) fail("progressive_raw_pilot_decision_invalid", "Pilot decision is invalid");
...
export function validateProgressiveRawCompleteReview(review, { plan = null } = {}) {
  return result(() => {
    ...
    } else if (!PROGRESSIVE_RAW_REVIEW_DECISIONS.includes(review.decision) || !review.previous_review_sha256) {
