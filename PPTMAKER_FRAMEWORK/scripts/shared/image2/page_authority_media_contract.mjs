...
const FINAL_MEDIA_BY_WORKFLOW = Object.freeze({
  pure: Object.freeze({ png: PAGE_AUTHORITY_NATIVE_RAW_PNG, requires_raw_byte_identity: true }),
  framed: Object.freeze({ png: PAGE_AUTHORITY_FRAMED_FINAL_PNG, requires_raw_byte_identity: false }),
});
...
export function pageAuthorityFinalPngForWorkflow(workflow) {
  const contract = FINAL_MEDIA_BY_WORKFLOW[workflow];
  if (!contract) throw new TypeError("Page Authority workflow has no final PNG contract");
  return contract.png;
}
...
export function inspectPageAuthorityFinalMedia({ workflow, finalBytes, rawSha256, finalSha256 } = {}) {
  const contract = FINAL_MEDIA_BY_WORKFLOW[workflow];
  if (!contract) return Object.freeze({ ok: false, code: "final_media_invalid" });
  const media = inspectExactPageAuthorityPng(finalBytes, contract.png);
