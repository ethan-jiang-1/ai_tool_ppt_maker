...
> 本模块是**能力层**——它不产出文件，而是赋能 [01 Visual Style Master](../01_visual_style_master/) 和 [02 Content Design](../02_content_design/) 中的 IMAGE PROMPT 写作。所有 IMAGE PROMPT 最终都写入 run bundle 的源文件中。
...
Image generation 的官方路径是 `unified_pipeline.mjs` → skill `image2-ppt/scripts/generate_full_page_images.py`。无 skill 时才用遗留参考 `stage2_generate_images.LEGACY.py`。参见 `00_project_setup/03-tool-selection.md` 了解 API 配置。
