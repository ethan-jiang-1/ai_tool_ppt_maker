...
4. **搭完后,框架里留了一份给你看的说明**(`deck-guide.md`)——告诉你"现在能改什么、怎么改"。之后想调整,继续跟 agent 说人话就行("第 5 页换个例子"、"颜色太暗了")。
...
生产完成后,你的项目目录(`deck_{NAME}/`)里会有一个 **`deck-guide.md`**——用大白话写的,一进去就告诉你:
- 你现在在哪一步
- 你能改哪些文件(只有两处是你的:内容和视觉主干)
- 改完怎么让它重新生成
- 下一步可以做什么
