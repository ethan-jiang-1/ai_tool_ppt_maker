...
The HTML renderer consumes the validated `html_first` projection. A style master is not a prerequisite for a new deck. Optional Image2 visual-slot refinement is considered only after current HTML delivery and exact cost authorization; markerless historical decks use `reference/legacy-image2-first-maintenance.md`.
