...
position: "04 of 05"
type: methodology
summary: "图片生成只用 GPT Image 2。API 配置（直接或中转均可）。参考脚本在 scripts/。"
depends_on:
  - "workflow/00-setup/README.md"
  - "workflow/00-setup/02-python-environment.md"
feeds_into:
...
`scripts/` 包含管线脚本 + 统一入口。**就地运行，不复制进 run bundle。** Stage 2 官方走 skill。
