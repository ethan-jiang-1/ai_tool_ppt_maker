 ...
 *
 *     node 00_project_setup/00-env-check.mjs           # human-readable
 *     node 00_project_setup/00-env-check.mjs --json    # machine-readable
 */
...
function checkFonts() {
  const bundled = join(resolve(__dirname, '..'), '06_reference_scripts', 'fonts');
  const searchDirs = [
    bundled,
  ...
  return {
    ...
    fix: found ? null : 'Optional. Drop SourceSansPro-*.otf into 06_reference_scripts/fonts/ (or set PPT_FONT_DIR).',
  };
}
