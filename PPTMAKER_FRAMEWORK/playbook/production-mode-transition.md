...
**Legacy adoption**: For a specifically targeted historical run, use only `state --inspect-legacy-protocol`, `state --prepare-legacy-adoption`, `state --preview-legacy-adoption`, `state --confirm-legacy-adoption --plan-hash <hash>`, `state --apply-legacy-adoption --plan-hash <hash>`, and the matching closed recovery forms. Only the observer's exact `recognized-legacy` result may prepare a fixed `image2-page-authority-v2` target. The Agent prepares the confined candidate shell. The human authors Page Authority source, chooses one target `framed|pure` workflow, records target intake, and supplies one exact adoption-matrix row for every legacy and target stable ID; preview all per-slide dispositions and the exact hash before confirmation. Adoption is provider-free and copies no legacy prompt, pixel, review, provider, raw/final, PPTX, notes, or delivery material. A published target starts at the selected target raw-authorization route with `source_epoch: 1` and `needs_raw_generation` for every target slide; Image2 quality review is a later, separate human decision.
