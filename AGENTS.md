...
├── deck_*/                    ← [产出] run bundle — Harness 生产出的 PPT 项目目录
...
本仓库只有 4 个目录是 PPT Maker Harness 源代码：

| Harness 目录 | 是什么 |
|----------|--------|
| `ppt_maker_harness/` | 方法论 + 管线脚本（soft bundle） |
| `openspec/` | 规格系统 |
| `tests/` | 单元/集成测试 |
| `tests_e2e/` | 端到端测试 |

**`deck_*` 和 `dpt_*` 是 Harness 的产出物**——`deck_*` 是 run bundle（`ppt_flow.mjs init` 创建，做 PPT 时 Agent 在里面工作），`dpt_*` 是 deep research 素材。它们是生产数据，不是 Harness 代码。

Agent 探索/理解 Harness 时只看上面 4 个源码目录。**做具体 deck 时由用户指定 deck 路径，然后在该 run bundle 内操作。**

## 关键约束

- 做具体 PPT 时 `ppt_maker_harness/` 是只读方法论；repo 维护 change 可以按 OpenSpec 任务修改 Harness 源
- Agent 拥有过程, 人类拥有内容
...
- 新增或修改 direct CLI、命令、exit path、stdout JSON、stderr diagnostic、child process、`cli_error.mjs`：先读 `openspec/specs/cli-surface/spec.md`，再用 `openspec status` 找 active `cli-surface` delta，并复用 `ppt_maker_harness/scripts/shared/cli/cli_error.mjs`。
...
- 上述是 Harness repository maintenance；run-bundle 生产 Agent 仍不得修改 Harness，也不得手改 `_generated/`。
