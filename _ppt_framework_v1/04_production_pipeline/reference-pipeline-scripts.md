...
> **此文件保留作为架构参考。** 完整的可运行 Python 实现已移至 `06_reference_scripts/` ——五个 Stage 的生产脚本，复制到 run bundle 后修改常量即可使用。
>
> 以下是六个关键模式的注释版伪代码——展示每个阶段的核心代码结构和设计决策。读它来理解**为什么这样设计**；用 `06_reference_scripts/` 来**实际执行**。
