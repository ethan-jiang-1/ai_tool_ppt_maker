...
参见 `03_image_prompts/02-prompt-structure-and-patterns.md`。
...
参见 `04_production_pipeline/00-the-pipeline-philosophy.md`。
...
参见 `02_content_design/05-iterate-with-version-discipline.md`（"什么是一个版本"）和 `00_project_setup/04-conventions.md`。
...
**修复**：用 `01_visual_style_master/04-iterate-review-lock.md` 里的 checklist 系统性审查。做一张 contact sheet 把 style master 和几张代表性 slides 放在一起对比。**Path A（95%+ pass）→ Lock。Path B（80-95%）→ 微调。Path C（<80%）→ 回到设计。** 不在 Path A 不要 lock。

参见 `01_visual_style_master/04-iterate-review-lock.md`。
...
- 不确定就查:`python _ppt_framework_v1/06_reference_scripts/bundle_layout.py`(打印权威树)。
- 校验一个 bundle 合不合规:`python _ppt_framework_v1/06_reference_scripts/bundle_layout.py --check deck_{NAME}/3_versions/v1`。管线每次运行前也会自动 check,结构不对直接拒绝。
