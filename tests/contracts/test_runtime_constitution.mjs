...
import { EXECUTABLE_INVENTORY } from '../../PPTMAKER_FRAMEWORK/scripts/shared/cli/cli_error.mjs';
...
describe('runtime constitution — Node only', () => {
  ...
  it('the explicit executable inventory matches direct-entry modules and excludes libraries', () => {
    const direct = walkFiles(SCRIPTS)
      .filter((path) => path.endsWith('.mjs'))
      .filter((path) => /cli_bootstrap\.mjs\?entry=/.test(readFileSync(path, 'utf8')))
  ...
  it('env-check reports in-framework stage2_generator as ok', () => {
    let out = '';
    try {
      out = execSync('node PPTMAKER_FRAMEWORK/scripts/00-setup/env-check.mjs --json --image2', {
        encoding: 'utf-8',
        timeout: 15000,
      });
    } catch (e) {
  ...
  it('unified_pipeline exports stage2 and no longer exports findSkillScript', async () => {
    const mod = await import('../../PPTMAKER_FRAMEWORK/scripts/03-html-production/unified_pipeline.mjs');
