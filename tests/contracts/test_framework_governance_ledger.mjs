...
const ledger = JSON.parse(readFileSync("tests/contracts/framework-governance-ledger-v1.json", "utf8"));
const FIELDS = ["classification", "disposition", "failure_story", "id", "invariant", "next_action", "owner", "source"];

function validateLedger(value) {
  const errors = [];
  if (value?.schema !== "pptmaker-framework-governance-ledger-v1" || !Array.isArray(value.rules)) return ["invalid schema or rules"];
  const ids = new Set();
...
describe("framework governance ledger", () => {
