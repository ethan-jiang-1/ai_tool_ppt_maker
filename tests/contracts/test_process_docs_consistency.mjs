...
describe("framework documentation coherence", () => {
  ...
  it("fixtures catch links, paths, version drift, hierarchy ambiguity, refresh routing, flags, and broad exceptions", () => {
    ...
    expect(validateRetiredWholePageTokenExceptions([{ token: "not-a-retired-token", file: "tests/contracts/test_process_docs_consistency.mjs", reason: "bad", owner: "framework-contracts", public_compatibility: false, retire_by: "not-applicable:negative-test" }]).length).toBeGreaterThan(0);
  ...
  it("keeps stable identity and renderer authorization guidance aligned", () => {
    ...
    expect(readFileSync("PPTMAKER_FRAMEWORK/scripts/README.md", "utf8")).toContain("contracts/executable_inventory.mjs");
  });
