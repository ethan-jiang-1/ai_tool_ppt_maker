...
import { acquireHtmlPublishLock, htmlOwnerRoot, releaseHtmlPublishLock } from '../PPTMAKER_FRAMEWORK/scripts/lib/html_object_store.mjs';
...
describe('HTML authoritative review evidence', () => {
  ...
  it('recovers every allowed gate-journal byte pair and rejects the forbidden ordering', async () => {
    ...
    try {
      ...
      const renderer = await import('../PPTMAKER_FRAMEWORK/scripts/lib/html_slide_renderer.mjs');
      const review = await import('../PPTMAKER_FRAMEWORK/scripts/lib/html_review_evidence.mjs');
      const stateApi = await import('../PPTMAKER_FRAMEWORK/scripts/lib/state.mjs');
