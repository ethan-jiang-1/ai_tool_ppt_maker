...
const BOOTSTRAP = join(SCRIPTS, "lib", "cli_bootstrap.mjs");
...
describe("cli_error", () => {
  ...
  it("recursively detects exactly the registered direct CLI candidates", () => {
    const exceptions = new Map([
      ["lib/cli_bootstrap.mjs", "shared bootstrap inspects argv but is not a public CLI"],
      ["lib/cli_error.mjs", "compatibility installer is a library API, not a direct entry"],
    ]);
  ...
  it("requires every direct executable to use the matching literal first import", () => {
    for (const entry of EXECUTABLE_INVENTORY) {
      ...
      expect(firstImport, entry).toBe(`import "./lib/cli_bootstrap.mjs?entry=${entry}";`);
    }
  });
  ...
  it("has explicit return-category audits for every executable and ppt_flow command", () => {
    ...
    const executableAudit = Object.fromEntries(EXECUTABLE_INVENTORY.map((entry) => [entry, {
      help: focused("tests/test_cli_error.mjs", "every registered executable has side-effect-free help"),
      usage: focused("tests/test_cli_error.mjs", "every registered executable has side-effect-free help"),
      contextual: focused({
        ...
        "unified_pipeline.mjs": "tests/test_unified_pipeline.mjs",
  ...
  it("shares process-global mode state and preserves write callbacks on success", () => {
    ...
    try {
      ...
      const helperUrl = pathToFileURL(join(SCRIPTS, "lib", "cli_error.mjs")).href;
  ...
  it("withholds credential, prompt, provider body, and stack sentinels from the full provider failure channel", () => {
    ...
    try {
      ...
      const imageUrl = pathToFileURL(join(SCRIPTS, "image_api_client.mjs")).href;
      const helperUrl = pathToFileURL(join(SCRIPTS, "lib", "cli_error.mjs")).href;
