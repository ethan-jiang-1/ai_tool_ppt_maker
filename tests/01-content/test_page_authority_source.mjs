...
describe("Page Image Workflow source", () => {
  ...
  it("keeps Pure header literals provider-visible and forbids a Framed preset", () => {
    ...
    const error = parseError(source({
      workflow: "pure",
      slides: [slide({ workflow: "pure", extra: "**FRAME PRESET**: standard-v1\n" })],
    }));
  ...
  it("requires an explicit Framed title and header preset", () => {
    const withoutPreset = source().replace("**FRAME PRESET**: standard-v1\n", "");
  ...
  it("rejects per-slide policy, hybrid, and retired protocol ingress before a receipt exists", () => {
    const perSlide = source({ slides: [slide({ extra: "**WORKFLOW**: pure\n" })] });
    const hybrid = source().replace("workflow: framed", "workflow: hybrid");
    const retired = source().replace("page-image-workflow-v1", "page-authority-image2-v2");
    ...
    expect(parseError(retired).issues[0]).toMatchObject({
