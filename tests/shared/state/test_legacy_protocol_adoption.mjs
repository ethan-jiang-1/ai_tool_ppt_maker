...
describe("legacy protocol adoption", () => {
  it("classifies only the four direct protocol outcomes without writes or provider work", () => {
    ...
    try {
      ...
      writeFileSync(join(legacy.runDir, "slide-specifications.md"), legacySource());
      const mismatched = readState(legacy.deck, { purpose: "observe", heal: false, runVersion: "v1" });
      mismatched.production_mode.by_version["3_versions/v1"] = { mode: "html-only" };
      writeState(legacy.deck, mismatched);
  ...
  it("rejects incomplete, derived, or drifted candidate material before state confirmation", () => {
    ...
    try {
      ...
      expect(() => prepareProductionModeTransition(value.runDir, { targetMode: "image2-page-authority" })).toThrow(/explicit legacy adoption/);
    } finally {
      rmSync(value.root, { recursive: true, force: true });
    }
  });
  ...
  it("keeps legacy-adoption uncertain recovery under the same exact journal owner", () => {
    ...
    try {
      ...
      writeFileSync(journalPath, JSON.stringify({
        ...
        source_execution_id: "exec-legacy-source",
        source_version: "v1",
        target_version: "v2",
        target_mode: "image2-page-authority",
      ...
      recordProductionModeTransitionRecoveryConfirmation(value.deck, {
        sourceRunVersion: "v1",
      ...
      expect(readState(value.deck, { purpose: "observe", heal: false, runVersion: "v1" })).toMatchObject({
        playbook: "create-deck",
        execution_id: "exec-legacy-source",
      });
    } finally {
      rmSync(value.root, { recursive: true, force: true });
    }
