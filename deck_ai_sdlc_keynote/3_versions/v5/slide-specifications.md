...
# Slide Specifications — AI 时代的信息加工变革 (v5)

> Pure workflow — Image2 paints the entire visual underlay including display text fields. VISUAL SCENE carries the per-slide non-text visual scene description. VISUAL BRIEF drives style choices (recipe, composition, motifs). No `no-readable-text` or `no-labels` constraints — pure workflow does not need them.
...
**BODY**: 软件只是第一个被掀翻的样本，传统企业紧随其后——这是一场关于"工作方式"的整体重写，不是一次工具升级。
...
**VISUAL SCENE**: minimal cover page generous empty cream paper single thin amber horizontal line one small amber dot
...
**BODY**: 2024 AI 帮你写完这一行，2025 AI 帮你写完这个函数，2026 AI 接住整个项目让你去睡觉——不是 hype，是加速度；而且正在溢出到所有信息加工领域。
...
**VISUAL SCENE**: left to right timeline three growing light sources candle lamp sun above three ascending code fragments on cream paper
...
**BODY**: 代码有编译器，0.1 秒就知道对不对；GitHub 有几十亿行代码当教材——进步快→资本涌入→模型越训越强→开始溢出到其他领域。软件只是第一个被学透的。
...
**VISUAL SCENE**: two panel split left compiler feedback loop green check marks waterfall right code fragments falling into knowledge base both converge at center amber core
...
**BODY**: 同一个 Agent 引擎，同一种工作方式——给出任务、Agent 执行、人验收。软件开发者已经在这条路上跑了三年，办公室白领才刚刚开始。软件的今天就是你的明天。
...
**VISUAL SCENE**: central amber core left branch dark terminal code symbols programmer silhouette right branch bright document office worker silhouette both face the same core
...
**BODY**: 专才→通才。AI 横跨产品、程序、测试、运维、汇报五域；人以一个领域为边界，跨出就得换人。这次不是帮你更快——是帮你做了。好消息是它超级能干，挑战是你还不会跟搭档配合。两个都是真的。
...
**VISUAL SCENE**: left right comparison a specialist programmer at a narrow desk with one domain of tools right side one amber glass ai partner with a book and open palm spanning five domains a shared desk between them calm work setting
...
**BODY**: 瀑布、V 模型、敏捷——旧地图画的都是人独自怎么走；现在多了一个通才搭档，旧地图不管用了。新地图还在画布上，只有几根试探线和一个问号。这不是恐慌，是摸索中的诚实。
...
**VISUAL SCENE**: a wall with three faded classic sdlc sketches waterfall v-model agile loop a human figure and an amber glass ai partner face forward toward a blank canvas on an easel with tentative pencil lines and a question mark
...
**BODY**: Deer Valley（02/2026，~40 人）「There is more uncertainty than certainty. Nobody has it all figured out.」→ Engelberg（06/2026，~60 人）「Not slides — production. The debate is over.」同一群人、同一个召集人、5 个月内一次集体认知跃迁。disruption 的速度，不是渐进改进。
...
**VISUAL SCENE**: left right split snowy mountains campfire tentative mood left green valley glowing dashboards confident mood right center thick amber arrow spanning the gap marking five months
...
**BODY**: Fowler「Nothing has hit with the magnitude of AI.」Beck「中层是我最担心的。」TDD 从"重要"变成"不可协商"的生存技能。Laura Tacho 12 万开发者数据：好团队用 AI 降 50% incidents，差团队翻倍——AI 是放大器。
...
**VISUAL SCENE**: two silhouettes at a fireside chat amber stage light three keywords floating behind
...
**BODY**: 人机关系从"操作者→工具"变成"委托人→执行者"。Mollick「I no longer steer. I commission.」瓶颈第一次从机器变成人——人能不能放心把方向盘交给搭档。Trust Gap。
...
**VISUAL SCENE**: interior view of a modern car cabin from back seat looking forward ai partner at driver seat hands on steering wheel human in passenger seat holding a task slip relaxed trusting posture both facing the road ahead
...
**VISUAL SCENE**: horizontal chain of seven linked nodes each node marked with a different narrow specialist role middle three nodes spanned by one amber glass ai partner with arms extended left figures point up right figures point down
...
**VISUAL SCENE**: a large funnel dense code flowing in at wide top single drip emerging at narrow bottom tiny human silhouette sits overwhelmed at exit
...
**VISUAL SCENE**: upper panel anxious human bending over checking each ai partner one by one overwhelmed lower panel same human relaxed at a control console setting guardrails the same ai partners work confidently inside amber frames
...
**VISUAL SCENE**: three layer geological cross section top tier green junior safe middle tier amber red squeezed middle management bottom tier gold senior architects
...
**VISUAL SCENE**: left traditional five layer management pyramid crumbling faintly right arrow points to three role badges side by side ic contributor dri owner player coach a small amber ai agent symbol connecting them as coordination layer
...
**VISUAL SCENE**: three columns side by side builders creators sellers acquirers measurers reporters the measurer column highlighted in amber showing it is being reshaped
...
**VISUAL SCENE**: minimal pause page calm centered spacious cream paper a single small centered amber dot or faint hand drawn arrow suggesting a gear shift
...
**VISUAL SCENE**: two parallel chains top sdlc seven nodes bottom bpm six nodes vertical dotted lines connecting nodes between them timelines behind converging at 2026 with an amber dot
...
**VISUAL SCENE**: two distinct agents inside a warm amber force field frame one seated calm with neat papers and a pen the other standing dynamic connecting pipes and instruments one human hand rests lightly on the frame edge trust not anxiety sandbox not cage
...
**VISUAL SCENE**: four horizontal stacked panels with hand drawn borders same amber wave passing through each layer
...
**VISUAL SCENE**: one central planner agent figure radiating thin sepia lines to three distinct named specialist agent figures each with their own posture one human figure stamping signing behind one agent
...
**VISUAL SCENE**: a large container vessel in fine sepia ink with a translucent digital twin blueprint overlaid on the hull showing sensor nodes data flows an amber ai core glows on the bridge of the ship
...
**VISUAL SCENE**: three column comparison left roman legion hierarchy pyramid hand drawn in sepia ink center modern corporate pyramid identical structure gray sepia right same pyramid transformed soft amber ripple through middle layers fading pure relay management tiers a thin amber direct connection line links top to bottom
...
**VISUAL SCENE**: three columns left builders creators right sellers acquirers center measurers reporters with arrow transformation from role to ai augmented role
...
**VISUAL SCENE**: two thin sepia lines flowing in from left and right merging at center into one thicker amber stream a few small partner figures stand at the waters edge watching the confluence
...
**VISUAL SCENE**: minimal closing page calm centered spacious cream paper a single small amber dot like a period or first spark of light
